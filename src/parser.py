...
# --- Parsing Function ---
def parse_apple_docs_json(json_string: str, logger: LoggerInterface) -> Optional[DocumentationMap]:
    ...
    for identifier, ref_data in references.items():
        ...
        relative_url = ref_data.get("url")

        if not isinstance(relative_url, str):
            logger.warning(f"Reference with Identifier '{identifier}' Missing URL. Skipping.")
            continue

        if not isinstance(item_title, str):
            logger.warning(f"Reference with Identifier '{identifier}' Missing Title. Skipping.")
            continue
        
        if item_title in parsed_docs:
            logger.warning(f"Duplicate Title '{item_title}' Found for Identifier '{identifier}'. Keeping First Occurrence.")
            continue

        if relative_url:
            full_url = BASE_URL + relative_url
            # Parse directory structure: remove leading '/' and split
            dir_components = [comp for comp in relative_url.strip('/').split('/') if comp]
            ...
            if identifier.startswith("doc://"):
                ...
                if path.startswith("com.apple.documentation/"):
                    ...
                    potential_url = f"{BASE_URL}/documentation/{domain}/{remaining_path}"
                else:
                ...
                logger.info(f"Constructed URL for '{item_title}' from Identifier: {potential_url}")
            else:
                logger.warning(f"Reference '{item_title}' (id: {identifier}) Missing 'url' and Identifier Format is Unknown. Skipping.")
